...
def lambda_handler(event, _):
    ...
    current_script_location = os.path.realpath(os.path.dirname(__file__))
    ...
    for record in event['Records']:
        ...
        if s3_bucket and s3_key:
            with open(os.path.join(current_script_location, 'queries.csv')) as f:
                manifest: tm.IDPManifest = tm.IDPManifest()
                queries_config = [ tm.Query(text=row) for row in f if row ]
